...
exports.getRecipeById = async (encryptedId) => {
	...
	//get recipe ingredients
	const ingredients = await IngredientService.getIngredientsByRecipeId(recipe.get().id);
	...
	return ingredientsForRecipe.findAll({
		...
		.then(async res => {
			for (let ingredientForRecipe of res) {
				let recipe = await this.getRecipeByDecryptedId(ingredientForRecipe.recipeId);
				recipes.push(recipe);
			}
			return recipes;
		})
...
exports.updateTotalsOfRecipeByIngredientId = async (ingredientId) => {
	...
	for (let recipe of recipes)
		this.updateTotalsOfRecipeById(recipe.id);
}
...
exports.updateTotalsOfRecipeById = async (recipeId) => {
	...
	for (let ingredientForRecipe of ingredients) {
		let ingredient = await IngredientService.getIngredientById(ingredientForRecipe.ingredientId);
	...
	totals.totalCalories = totals.totalProtein * 4 + totals.totalCarbs * 4 + totals.totalFat * 9;
